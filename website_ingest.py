...
def load_website_chunks(user_id):
    ...
    # Used to read data/websites/{user_id}.txt directly - website_manager
    # (this business's configured root URL(s)) moved to Postgres, so this
    # goes through its get_websites() instead of a local file.
    root_urls = get_websites(user_id)

    if not root_urls:
        print("⚠️ No websites configured")
        return []

    # Each business only ever has one root URL configured (see
